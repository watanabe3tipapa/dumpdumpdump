...
### 1980's カセットデッキ調表示（v2 追加）
- トグル「1980's カセットデッキ調」で VU メーター風に切替
- 配色: 緑→黄→赤の 10 段 LED セグメント（VU_COLORS）、カラーバーも VU 表示
- 描画: セル間に 1px の暗色マス目（LED マトリクス風）＋行ごとの最大値にピークホールド線（麦色）
- パネル: テープカウンタ（hover/クリックでアドレス表示）+ 12 セグメント LEVEL メーター
  - LEVEL メーターは hover 中のページ値に応じて点灯、REC ランプはデッキモードで点灯
...
## レビュー/評価ログ（2026-08-10 続き）

- 複数行にまたがる領域のハイライト矩形が欠けるバグを検出・修正
  （width=|bx-ax| 方式 → 同一行は x=(p0%COLS)*CELL、複数行は x=0,width=COLS*CELL で全幅を覆う方式）
- 領域/マーカー不透明度スライダーが未適用だった問題を修正（fill/stroke-opacity, g@opacity に反映）
